...
public class CategoryException extends RuntimeException {
    ...
    public CategoryException(ResponseCode status, ErrorCode code) {
        super(errorMsg.formatted(status.message(), code.errorMsg()));
        this.status = status;
        this.code = code;
    }

    public CategoryException(ResponseCode status, ErrorCode code, String name) {
        super(errorMsgName.formatted(status.message(), code.errorMsg(), name));
        this.status = status;
        this.code = code;
    }

    public CategoryException(ResponseCode status, ErrorCode code, Long id) {
